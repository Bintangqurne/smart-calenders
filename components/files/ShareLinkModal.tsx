...
export function ShareLinkModal({ file, onClose }: Props) {
  const [expiresIn, setExpiresIn] = useState(PRESETS[2].seconds);
  const [password, setPassword] = useState("");
  const [permission, setPermission] = useState<"view" | "download">("view");
  const [info, setInfo] = useState<FileShareInfo | null>(null);
  ...
  async function handleCreate() {
    ...
    setCreating(true);
    try {
      const data = await createFileShareLink(file.fileId, {
      ...
      <div className="w-[min(500px,95vw)] overflow-hidden rounded-xl bg-white shadow-2xl dark:bg-slate-900">
        ...
        <div className="space-y-4 p-4">
          <div className="text-xs text-slate-500">
            File: <span className="font-medium text-slate-800 dark:text-slate-200">{file.fileName}</span>
          </div>

          {info ? (
            <div className="space-y-3">
              <div className="rounded-md bg-emerald-50 p-2 text-xs text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">
                Link created. Expires {new Date(info.expiresAt).toLocaleString()}.
            ...
            <>
              <div>
                <label className="mb-1 block text-xs font-medium">Expires in</label>
